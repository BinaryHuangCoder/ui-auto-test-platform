...
-- 测试任务与用例关联表
-- =====================================================
DROP TABLE IF EXISTS `test_task_case`;
CREATE TABLE `test_task_case` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `task_id` bigint(20) NOT NULL COMMENT '任务ID',
  `case_id` bigint(20) NOT NULL COMMENT '用例ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
-- 测试任务执行历史表
-- =====================================================
DROP TABLE IF EXISTS `test_task_execution`;
CREATE TABLE `test_task_execution` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '执行ID',
  `task_id` bigint(20) NOT NULL COMMENT '任务ID',
  `task_no` varchar(50) DEFAULT NULL COMMENT '任务编号',
  `task_name` varchar(200) DEFAULT NULL COMMENT '任务名称',
...
-- 测试任务执行步骤详情表
-- =====================================================
DROP TABLE IF EXISTS `test_task_step_execution`;
CREATE TABLE `test_task_step_execution` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '步骤执行ID',
  `task_execution_id` bigint(20) NOT NULL COMMENT '任务执行记录ID',
  `case_execution_id` bigint(20) NOT NULL COMMENT '用例执行记录ID',
  `step_id` bigint(20) DEFAULT NULL COMMENT '步骤ID',
...
-- 模型使用场景配置表
-- =====================================================
DROP TABLE IF EXISTS `sys_model_scenario`;
CREATE TABLE `sys_model_scenario` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='模型使用场景配置表';
